  ...
  if (results.length != 0)
    if (results.length > 9)
      ...
      $.each( results, (index, el) ->
        a = $("<a href='#' class='AutoPhoneClick'><span class='label'>"+el.phone+" "+el.firstname+" "+el.lastname+"</span></a>").data({phone: el.phone, firstname: el.firstname, lastname: el.lastname})
