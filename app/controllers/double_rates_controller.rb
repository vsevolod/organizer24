  ...
  def by_week
    ...
    ((@start.to_date)..(@end.to_date)).each_with_index do |t, index|
      ...
      @period += double_rates.where('week_day = :week_day OR day = :current_day', {week_day: current_day.wday, current_day: current_day}).to_a.map do |double_rate|
        {:start => (current_day+double_rate.begin_time.to_i).iso8601, :end => (current_day+double_rate.end_time.to_i).iso8601, :editable => false, title: "x#{double_rate.rate}", 'data-inner-class' => 'legend-rate'}
      end
    end
    respond_with(@period)
  end
