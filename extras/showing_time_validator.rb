...
class ShowingTimeValidator < ActiveModel::EachValidator
  ...
  def validate_each(record, _attribute, _value)
    ...
    duplicate_record = record.class.where(<<-SQL, start: start.utc.to_s, stop: (start + showing_time * 60).utc.to_s).where(:status => %w(offer approve taken), :id.not_eq => record.id, :organization_id => record.organization_id, :worker_id => record.worker_id)
